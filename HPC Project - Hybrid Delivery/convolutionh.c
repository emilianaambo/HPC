               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    #pragma omp parallel for schedule(dynamic) private (i, j, m, n, rowMax, rowMin,colMin , colMax)
...
int main(int argc, char **argv)
{
   // variable declarations
    ...
    ImagenData chunk_mem=NULL, chunk_memOUT=NULL;
    ...
    if (idprocess == 0){
        ...
        while (c < partitions) {
            ...
            task = source->altura/numproc;
            chunk_div = source->ancho*task;
            tag = 1;
            sendArr[0]=chunk_div;
            sendArr[1]=halosize;
            for (i=1;i<numproc; i++){
            MPI_Send(sendArr,2,MPI_INT,i,tag, MPI_COMM_WORLD);
            }
            //assigning  chunk + pointer
            pointerR = source->R+chunk_div;
            pointerG = source->G+chunk_div;
            pointerB = source->B+chunk_div;
            
           // Master sending image chunks to slaves
            for (i=1;i<numproc; i++){
                MPI_Send(pointerR,chunk_div,MPI_INT,i,tag, MPI_COMM_WORLD);
                MPI_Send(pointerG,chunk_div,MPI_INT,i,tag, MPI_COMM_WORLD);
                MPI_Send(pointerB,chunk_div,MPI_INT,i,tag, MPI_COMM_WORLD);
                //updating pointer
                pointerR+=chunk_div;
                pointerG+=chunk_div;
                pointerB+=chunk_div;
            ...
            convolve2D(source->R, output->R, source->ancho, (source->altura/partitions/numproc)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
             convolve2D(source->G, output->G, source->ancho, (source->altura/partitions/numproc)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
             convolve2D(source->B, output->B, source->ancho, (source->altura/partitions/numproc)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
            ...
            printf("%d", chunk_div);
            //Master receiving results from slaves
            for (i=1;i<numproc ;i++){
            MPI_Recv( output->R+i*chunk_div,chunk_div,MPI_INT,i,1, MPI_COMM_WORLD, &status);
            MPI_Recv( output->G+i*chunk_div,chunk_div,MPI_INT,i,2, MPI_COMM_WORLD, &status);
            MPI_Recv( output->B+i*chunk_div,chunk_div,MPI_INT,i,3, MPI_COMM_WORLD, &status);
        ...
        // Receive from Master
        tag = 1;
        MPI_Recv(sendArr,2,MPI_INT,0,tag, MPI_COMM_WORLD, &status);
        chunk_div= sendArr[0];
        halosize = sendArr[1];
        printf("Chunk_div: %d \n", chunk_div);
       //allocating memory for convolution input
        chunk_mem=(ImagenData) malloc(sizeof(struct imagenppm));
        chunk_mem->R=calloc(chunk_div,sizeof(int));
        chunk_mem->G=calloc(chunk_div,sizeof(int));
        chunk_mem->B=calloc(chunk_div,sizeof(int));
        //allocating memory for convolution output
        chunk_memOUT=(ImagenData) malloc(sizeof(struct imagenppm));
        chunk_memOUT->R=calloc(chunk_div,sizeof(int));
        chunk_memOUT->G=calloc(chunk_div,sizeof(int));
        chunk_memOUT->B=calloc(chunk_div,sizeof(int));
        //slave recieving image chunk from Master
        MPI_Recv(chunk_mem->R,chunk_div,MPI_INT,0,tag, MPI_COMM_WORLD, &status);
        MPI_Recv(chunk_mem->G,chunk_div,MPI_INT,0,tag, MPI_COMM_WORLD, &status);
        MPI_Recv(chunk_mem->B,chunk_div,MPI_INT,0,tag, MPI_COMM_WORLD, &status);
        #pragma omp parallel num_threads(THREADS)
        {
            #pragma omp sections nowait
            {
            #pragma omp section
                {        convolve2D(chunk_mem->R, chunk_memOUT->R, source->ancho, (source->altura/partitions/numproc)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
                }
            #pragma omp section
                {
        convolve2D(chunk_mem->G, chunk_memOUT->G, source->ancho, (source->altura/partitions/numproc)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
                }
                    
          #pragma omp section
           {
        convolve2D(chunk_mem->B, chunk_memOUT->B, source->ancho, (source->altura/partitions/numproc)+halosize, kern->vkern, kern->kernelX, kern->kernelY);
          }
       }
     }
     	  //slave sending image chunk
        MPI_Send(chunk_memOUT->R,chunk_div,MPI_INT,0,1, MPI_COMM_WORLD);
        MPI_Send(chunk_memOUT->G,chunk_div,MPI_INT,0,2, MPI_COMM_WORLD);
        MPI_Send(chunk_memOUT->B,chunk_div,MPI_INT,0,3, MPI_COMM_WORLD);
