...
int main(int argc, char **argv)
{
    ...
    if (idprocess == 0){
        ...
        while (c < partitions) {
            ...
            output->B+=chunk_div;
            for (i=1;i<numproc; i++){
            MPI_Recv( output->R,chunk_div,MPI_INT,i,1, MPI_COMM_WORLD, &status);
            MPI_Recv( output->G,chunk_div,MPI_INT,i,2, MPI_COMM_WORLD, &status);
            MPI_Recv( output->B,chunk_div,MPI_INT,i,3, MPI_COMM_WORLD, &status);
